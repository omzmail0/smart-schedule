      ...
      {logic.view === 'login' ? (
        ...
        <>
          ...
          <div className="p-5 max-w-lg mx-auto pb-24">
            ...
            {logic.activeTab === 'settings' && logic.user.role === 'admin' && (
              <SettingsTab 
                    settings={logic.settings} 
                    setSettings={logic.setSettings} 
                    saveSettings={logic.saveSettings} 
                    resetAllAvailability={logic.resetAllAvailability} 
              />
            )}

            {logic.activeTab === 'analysis' && (
               <AnalysisTab 
                    settings={logic.settings} 
                    analyzeSchedule={logic.analyzeSchedule} 
                    analysisResult={logic.analysisResult} 
                    bookMeeting={logic.bookMeeting} 
               />
            )}
          ...
          <MemberModal 
            ...
            isEditing={!!logic.editingMemberId} 
            form={logic.memberForm} 
            setForm={logic.setMemberForm} 
            onSave={logic.handleSaveMember} 
            primaryColor={logic.settings.primaryColor} 
          />

          <InspectModal 
            member={logic.inspectMember} 
            onClose={() => logic.setInspectMember(null)} 
            primaryColor={logic.settings.primaryColor} 
            availability={logic.availability} 
          />
